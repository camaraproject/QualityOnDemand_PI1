...
@Repository
@Profile("!local")
public interface QodSessionRedisRepository extends CrudRepository<RedisQosSession, UUID> {
   ...
   */
  List<RedisQosSession> findByDeviceIpv4addr(String ipAddr);
   ...
   * @return QoS session or null if session not found
   */
  Optional<RedisQosSession> findBySubscriptionId(@NotBlank String subscriptionId);
}
