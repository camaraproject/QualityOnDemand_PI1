...
@Slf4j
public class AsIdConverter implements AttributeConverter<AsId, String> {

  private static final String ERROR_MESSAGE = "Error during conversion of entity.";
  private static final ObjectMapper objectMapper = new ObjectMapper();

  @Override
  public String convertToDatabaseColumn(AsId attribute) {
    if (null == attribute) {
      return null;
    }
    try {
  ...
  @Override
  public AsId convertToEntityAttribute(String dbData) {
    try {
      return objectMapper.readValue(dbData, AsId.class);
    } catch (JsonProcessingException e) {
      log.error(e.getMessage());
      throw new SessionApiException(HttpStatus.INTERNAL_SERVER_ERROR, ERROR_MESSAGE);
    }
