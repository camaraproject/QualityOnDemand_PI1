...
import static com.camara.exception.ErrorCode.VALIDATION_FAILED;
...
public class ExceptionHandlerAdvice {
  ...
  /**
   * This function handles {@link ConstraintViolationException}.
   *
   * @param ex The exception {@link ConstraintViolationException}, caused by validation failures in path variables, request parameters,
   *           etc.
   * @return HTTP response with the status 400 - Bad Request
   */
  @ExceptionHandler(ConstraintViolationException.class)
  public ResponseEntity<ErrorInfo> handleException(ConstraintViolationException ex) {
    log.error("ConstraintViolationException raised: {}", ex.getMessage());

    // Get the first violation (or handle multiple violations if needed)
    ConstraintViolation<?> violation = ex.getConstraintViolations().stream().findFirst().orElse(null);

    String message = "Validation failed";
    if (violation != null) {
      // Extract the field causing the violation and the detailed message
      String value = violation.getInvalidValue().toString();
      message = String.format("Validation failed for property <%s> with value <%s>", violation.getPropertyPath().toString(), value);
    }
  ...
  public ResponseEntity<ErrorInfo> handleException(ValueInstantiationException ex) {
    ...
    return ResponseEntity.badRequest()
        ...
        .body(new ErrorInfo()
            .status(HttpStatus.BAD_REQUEST.value())
            .code(VALIDATION_FAILED.name())
            .message("Validation failed for parameter '" + ex.getPath().getFirst().getFieldName())
        );
  }
  ...
  public ResponseEntity<ErrorInfo> handleException(HttpMessageNotReadableException ex) {
    ...
    switch (throwable) {
      case JsonEOFException exception -> {
        log.debug(exception.getMessage());
        errorMessage = "Invalid json: start '{' or end-braces '}'";
      }
      case JsonParseException jsonParseException -> errorMessage = jsonParseException.getOriginalMessage();
      case UnrecognizedPropertyException unrecognizedPropertyException -> {
        ...
        if (!jsonReferences.isEmpty()) {
          String invalidField = jsonReferences.getFirst().getFieldName();
      ...
      case null, default -> errorMessage = "Undefined HTTP Message not readable exception" + errorMessage;
    }
    return ResponseEntity.badRequest().body(
        new ErrorInfo()
            .status(HttpStatus.BAD_REQUEST.value())
  ...
  public ResponseEntity<ErrorInfo> handleException(HttpMediaTypeNotAcceptableException ex) {
    log.error("Unacceptable media-type exception occurred: {}", ex.getMessage());
